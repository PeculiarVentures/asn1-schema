import * as assert from "assert";
import { AsnConvert } from "@peculiar/asn1-schema";
import { CertificationRequest } from "@peculiar/asn1-csr";

context("csr", () => {

  it("parse simple request", () => {
    const pem = "";
    const csr = AsnConvert.parse(Buffer.from(pem, "base64"), CertificationRequest);

    assert.equal(csr.certificationRequestInfo.subject[0].length, 0);
    assert.equal(csr.certificationRequestInfo.version, 0);
    assert.equal(csr.certificationRequestInfo.attributes.length, 0);
    assert.equal(csr.signature.byteLength, 256);
  });

});